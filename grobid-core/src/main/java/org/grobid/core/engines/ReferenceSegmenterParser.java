...
public class ReferenceSegmenterParser extends AbstractParser implements ReferenceSegmenter {
    ...
    public List<LabeledReferenceResult> extract(Document doc, SortedSet<DocumentPiece> referencesParts,
        ...
        try {
            ...
            // to support long sequence in case of RNN usage we segment in pieces of less
            // than the
            ...
            // this is relevant to the reference segmenter RNN model, which is position-free
            // in its
            // application, but could not be generalized to other RNN or transformer model
            // long inputs
            GrobidCRFEngine grobidEngine = GrobidProperties.getGrobidEngine(REFERENCE_SEGMENTER);
            if (grobidEngine == GrobidCRFEngine.DELFT || grobidEngine == GrobidCRFEngine.ONNX) {
                ...
                if (grobidEngine == GrobidCRFEngine.DELFT && GrobidProperties.getInstance()
                    .getDelftRuntimeMaxSequenceLength(REFERENCE_SEGMENTER.getModelName()) != -1) {
                    originalMaxSequence = GrobidProperties.getInstance()
                        .getDelftRuntimeMaxSequenceLength(REFERENCE_SEGMENTER.getModelName());
                } else if (grobidEngine == GrobidCRFEngine.ONNX) {
                    int configMaxSeq = GrobidProperties
                        .getOnnxRuntimeMaxSequenceLength(REFERENCE_SEGMENTER.getModelName());
                    if (configMaxSeq != -1) {
                        originalMaxSequence = configMaxSeq;
                    } else {
                        int modelMaxSeq = DeLFTOnnxModel
                            .readMaxSequenceLength(Paths.get(REFERENCE_SEGMENTER.getModelPath()));
                        if (modelMaxSeq != -1) {
                            originalMaxSequence = modelMaxSeq;
                        }
                    }
                }
                ...
                if (featureVectorLines.length < originalMaxSequence || originalMaxSequence < 600) {
                    ...
                    // if the max sequence length is too small, e.g. transformer, we won't be able
                    // to manage
                    ...
                    // System.out.println("originalMaxSequence: " + originalMaxSequence + " /
                    // maxSequence adjusted to: " + maxSequence);

                    List<List<String>> featureVectorPieces = new ArrayList<>();
                    // segment the input vectors in overlapping sequences, according to the model
                    // max_sequence_length parameter
                    for (int i = 0; (i * maxSequence) < featureVectorLines.length; i++) {
                        ...
                        // System.out.println("lowerBound: " + lowerBound + " - upperBound: " +
                        // upperBound);
                    ...
                    /*
                     * System.out.println("featureVectorPieces.size(): " +
                     * featureVectorPieces.size());
                    ...
                    // segment this result to get back the input chunk alignment (with extra 500
                    // overlaping lines)
                        ...
                        for (int j = previousTransitionPos; j < transitionPos; j++) {
                            if (j == previousTransitionPos && previousTransitionPos != 0) {
                                ...
                                if (localLine.indexOf(TaggingLabels.GROBID_START_ENTITY_LABEL_PREFIX) == -1) {
                                    localLine = localLine.replace("<label>",
                                        TaggingLabels.GROBID_START_ENTITY_LABEL_PREFIX + "<label>");
                                    localLine = localLine.replace("<reference>",
                                ...
                                if (localLine.indexOf(TaggingLabels.GROBID_START_ENTITY_LABEL_PREFIX) != -1) {
                                    localLine = localLine.replace(
                                        TaggingLabels.GROBID_START_ENTITY_LABEL_PREFIX + "<label>", "<label>");
                                    localLine = localLine.replace(
                                        TaggingLabels.GROBID_START_ENTITY_LABEL_PREFIX + "<reference>",
                                        "<reference>");
                                }
